package vn.ngphong.musiccc.ui.fragments
...
class PlaylistTracksFragment : BaseFragment() {
    ...
    private var listPlaylistTracks = mutableListOf<Song>()
    private var listAllTracks = mutableListOf<Song>()
    ...
    override fun onCreateView(
        ...
        stringToList()
        listAllTracks = DataLoader(requireContext()).querySongs()
        val toRemove = mutableListOf<Song>()
    ...
    private fun stringToList() {
        if (listTracksString != null) {
            ...
            val tracks = gson.fromJson<MutableList<Song>>(listTracksString, type)
...
//            val gsonString = gson.toJson(listPlaylistTracks)
    ...
    interface OnSaveListener {
        fun onSaveClicked(list: MutableList<Song>, pos: Int)
    }
}