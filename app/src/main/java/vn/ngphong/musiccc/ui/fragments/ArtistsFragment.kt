...
import vn.ngphong.musiccc.adapters.ArtistAdapter
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        artistAdapter!!.setOnClickListener(object : ArtistAdapter.OnClickListener {
            override fun onPlayIconClick(position: Int) {
                iPlayerHolder!!.updateTracks(listArtists[position].songs, 0)
                iPlayerHolder!!.play()
                mainActivity!!.updateFooter()
            }

            override fun onItemTrackClick(position: Int, parentPosition: Int) {
                iPlayerHolder!!.updateTracks(listArtists[parentPosition].songs, position)
                iPlayerHolder!!.play()
                mainActivity!!.updateFooter()
            }
